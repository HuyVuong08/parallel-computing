...
def download_rodinia_and_extract():
    ...
    command = f'wget http://www.cs.virginia.edu/~kw5na/lava/Rodinia/Packages/Current/rodinia_3.1.tar.bz2 && tar -xf ./rodinia_3.1.tar.bz2 rodinia_3.1/data && mv ./rodinia_3.1/data {SRC_DIR}/data'
...
def run_setup_command_for_file(targetFile, command, targetDir=None):
    """
    Run a setup command in the downloads directory (or provided directory) if the
    expected file does not already exist.
    """
    if targetDir is None:
        targetDir = DOWNLOAD_DIR

    if not os.path.isfile(targetFile):
        assert targetDir, "targetDir must be set before running setup commands"
    ...
    for target in targets:
        ...
        if basename == 'gc-cuda':
            ...
            if not os.path.isfile(f'{srcDir}/d493.tsp'):
                # Try primary TSPLIB host, then a GitHub mirror; if both fail, generate a tiny TSPLIB file locally.
                attempts = [
                    ('http://comopt.ifi.uni-heidelberg.de/software/TSPLIB95/tsp/d493.tsp.gz', True),
                    ('https://raw.githubusercontent.com/rklyne/tsplib/master/TSPLIB95/tsp/d493.tsp', False),
                ]
                success = False

                for url, is_gz in attempts:
                    subprocess.run('rm -f d493.tsp d493.tsp.gz', cwd=DOWNLOAD_DIR, shell=True)
                    if is_gz:
                        cmd = f'wget --no-check-certificate {url} -O d493.tsp.gz && gunzip -f d493.tsp.gz'
                    else:
                        cmd = f'wget --no-check-certificate {url} -O d493.tsp'
                    result = subprocess.run(cmd, cwd=DOWNLOAD_DIR, shell=True)
                    if result.returncode == 0 and os.path.isfile(f'{DOWNLOAD_DIR}/d493.tsp'):
                        success = True
                        break

                if not success:
                    print('[WARN] Failed to download d493.tsp; generating synthetic TSPLIB instance.')
                    tsp_contents = (
                        "NAME: d493\n"
                        "TYPE: TSP\n"
                        "DIMENSION: 5\n"
                    ...
                    with open(f'{DOWNLOAD_DIR}/d493.tsp', 'w') as fp:
                        fp.write(tsp_contents)

                subprocess.run(f'mv d493.tsp {srcDir}/', cwd=DOWNLOAD_DIR, shell=True, check=True)
            ...
            if not os.path.isfile(f'{srcDir}/alphas.csv'):
                # generate synthetic dataset matching expected dimensions (n=26280, K=21)
                command = (
                    'python3 - <<\"PY\"\n'
                    'import random\n'
                    'n, K = 26280, 21\n'
                    'with open("alphas.csv", "w") as f:\n'
                    '    for _ in range(n * K):\n'
